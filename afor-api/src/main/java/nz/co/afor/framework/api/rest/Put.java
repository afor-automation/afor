...
public class Put extends AbstractHttpRequest {
    ...
    public <T> ResponseEntity<T> request(URI uri, Object request, Class<T> responseType) {
        log.info(String.format("Sending PUT request to URL '%s'", uri.toString()));
        @SuppressWarnings("unchecked") HttpEntity entity = new HttpEntity(request, getHeaders());
        ResponseEntity<T> response = getRestTemplate().exchange(uri, HttpMethod.PUT, entity, responseType);
